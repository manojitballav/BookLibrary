...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if the_user is not None and the_user.verify_password(login_form.password.data):
            ...
            flash(u'Logged In!  Welcome %s!' % the_user.name, 'success')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Welcome %s!' % form.name.data, 'success')
        login_user(the_user)
        return redirect(request.args.get('next') or url_for('main.index'))
    return render_template('register.html', form=form, title=u"New User")
